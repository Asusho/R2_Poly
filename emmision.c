...
char interval = 96;
char front_to_send = 0; // Le nombre de front � envoyer, s'il est modifi�

// D�lai d'attente de n * 50 it�rations de boucle
void delai (char n) {
	while (n > 0) {
		char i = 0;
		while (i < 50) i++;
...
// Envoie un certain nombre de fronts dans l'�metteur
void front (char n) {
...
void main(void){
	PORTA = 0;
    PORTB = 0;
    TRISB = 0b11111111; // Port B en entr�e
